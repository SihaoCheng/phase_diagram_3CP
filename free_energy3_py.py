...
def df_OCP(gamma):
    ...
    return (gamma<100.0) * (-0.3683 + 0.00458*(gamma-100.0)) + \
           (gamma>=100) * (gamma<=200) * (-0.003243*gamma + 1.8645*gamma**0.32301 - 1.7748*np.log(gamma) - 0.2316 + 10.84/gamma) +\
           (gamma>200.0) * (0.09384 + 0.00428*(gamma-200.0))

def is_liquid(gamma1, x1, x2, RZ2, RZ3):
...
def Smix(x1, x2, RZ2, RZ3):
    ...
    Smix_val = 0.0
    if x1>0.0:
        Smix_val += x1*np.log(x1/xbar)
    if x2>0.0:
        Smix_val += x2*np.log(x2*RZ2/xbar)
    if x3>0.0:
        Smix_val += x3*np.log(x3*RZ3/xbar)
    return Smix_val

def f_min(gamma1, x1, x2, RZ2, RZ3):
...
def f_liquid(gamma1, x1, x2, RZ2, RZ3):
